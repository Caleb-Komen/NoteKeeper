...
import static com.foozenergy.notekeeper.NoteKeeperDatabaseContract.CourseInfoEntry;
...
public class DataManager {
    ...
    public static void loadFromDatabase(NoteKeeperOpenHelper openHelper){
        ...
        String[] courseColumns = new String[]{CourseInfoEntry.COLUMN_COURSE_ID, CourseInfoEntry.COLUMN_COURSE_TITLE};
        SQLiteDatabase db = openHelper.getReadableDatabase();
        Cursor courseCursor = db.query(CourseInfoEntry.TABLE_NAME, courseColumns,
                null, null, null, null, CourseInfoEntry.COLUMN_COURSE_TITLE);
        loadCoursesFromDatabase(courseCursor);

        String[] noteColumns = new String[]{NoteInfoEntry.COLUMN_NOTE_TITLE,NoteInfoEntry.COLUMN_NOTE_TEXT,
    ...
    private static void loadNotesFromDatabase(Cursor cursor) {
        ...
        while (cursor.moveToNext()){
            String noteTitle = cursor.getString(cursor.getColumnIndex(NoteInfoEntry.COLUMN_NOTE_TITLE));
            String noteText = cursor.getString(cursor.getColumnIndex(NoteInfoEntry.COLUMN_NOTE_TEXT));
            String courseId = cursor.getString(cursor.getColumnIndex(NoteInfoEntry.COLUMN_COURSE_ID));
            int id = cursor.getInt(cursor.getColumnIndex(NoteInfoEntry._ID));
    ...
    private static void loadCoursesFromDatabase(Cursor cursor) {
        ...
        DataManager dm = DataManager.getInstance();
        //clear the course list of any previous data
        dm.mCourses.clear();
        while(cursor.moveToNext()){

            String courseId = cursor.getString(cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_ID));
            String courseTitle = cursor.getString(cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_TITLE));
            CourseInfo course = new CourseInfo(courseId,courseTitle,null);

            dm.mCourses.add(course);
        }
        //close cursor to avoid system resource leak
        cursor.close();
    }
    ...
    public int createNewNote(CourseInfo course, String title, String text) {
        int noteIndex = createNewNote();

        NoteInfo note = DataManager.getInstance().getNotes().get(noteIndex);
